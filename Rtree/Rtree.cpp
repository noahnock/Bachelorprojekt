...
std::pair<OrderedBoxes, OrderedBoxes> OrderedBoxes::SplitAtBestOnDisk(const std::string& filePath, size_t S, long long maxBuildingRamUsage) {
    OrderedBoxes split0;
    OrderedBoxes split1;
...
std::pair<OrderedBoxes, OrderedBoxes> OrderedBoxes::SplitAt(const std::string& filePath, size_t dim, long long index, boxGeo boundingBox, long long maxBuildingRamUsage) {
    ...
    if (this->workInRam) {
        ...
        std::remove(this->rectanglesD1OnDisk.c_str());
        if (!split0InRam) {
            split0.CreateOrderedBoxesOnDisk(filePath + ".0.dim0", filePath + ".0.dim1", index);
        } else {
            split0.CreateOrderedBoxesInRam(bestDimSplit0OnRam, otherDimSplit0OnRam);
        }

        if (!split1InRam) {
            split1.CreateOrderedBoxesOnDisk(filePath + ".1.dim0", filePath + ".1.dim1", this->size - index);
        } else {
